      ...
      <?php foreach ($menuList as $navItem): ?>
        <?php if (in_array(reset($_SESSION)['role_name'], $navItem['nav-permission'])): ?>
          <li class="nav-item">
            ...
            <a data-bs-toggle="
            <?php if ($navItem['has-child']) {
              echo 'collapse';
            } ?>" href="<?php if ($navItem['has-child']) {
               echo '#' . $navItem['nav-id'];
             } else {
               echo getPageFloor(0) . $navItem['nav-link'];
             } ?>" class="nav-link <?php if (isUrlActive($navItem['nav-link']))
            ...
            <div class="collapse <?php if (isUrlActive($navItem['nav-link']))
                ...
                <?php foreach ($navItem['nav-child'] as $navChildItem): ?>
                  <?php if (in_array(reset($_SESSION)['role_name'], $navChildItem['nav-permission'])): ?>
                    <li class="nav-item ">
                      ...
                      <div class="collapse <?php if (isUrlActive($navChildItem['nav-link']))
